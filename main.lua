...
colored_lines = V2
cursor_pos = read_var("cursorpos")
cursor = V3
lines_to_update = V4
current_line = read_var("currentline")
update_all_lines = V5
if update_all_lines == true then
	lines_to_update = {}
	for i, _ in pairs(lines) do table.insert(lines_to_update, i) end
elseif lines_to_update == nil then
	lines_to_update = {current_line}
end
...
function cur(text, line_no, output_index)
	local res
	if output_index <= cursor_pos and (output_index + #text) > cursor_pos and line_no == current_line then
...
function PROCESS(text, line_no)
	...
	while #text > 0 do
		...
		if pattern_name == nil then
			...
			t, index = cur(text:sub(1,1), line_no, index)
			result = result .. t
			...
			t, index = cur(match_text, line_no, index)
			result = result .. t
...
--PARSE UPDATED LINES
for _, i in pairs(lines_to_update) do
	if lines[i] ~= nil then
		colored_lines[i] = PROCESS(lines[i], i)
	end
end

output_array(colored_lines, 1)